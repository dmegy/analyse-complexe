...
\begin{theoreme}[Conditions de Cauchy-Riemann]
...
\end{proposition}
Dans la plupart des cas, cette majoration brutale avec la longueur du chemin et la norme infinie de $f$ sur le support du chemin suffit. Toujours essayer ceci en priorité. S'il faut quelque chose de plus subtil, revenir à la version précédente.
...
\begin{attention}
L'énoncé suivant est \textbf{FAUX} : \og Soit $f\in \mathcal O(U)$ et $\gamma$ un lacet de $U$. Alors $\int_\gamma f(z)dz = 0$\fg. Voir le document \og idées fausses\fg.
\end{attention}
...
\end{theoreme}
 ...
 (C'est juste la formule de Cauchy d'ordre $n$, pour des disques centrés en zéro).

\subsection{Séries de Laurent}

Définition, couronne de convergence, dérivée.
...
\begin{proposition}[Convergence des produits de Cauchy]
