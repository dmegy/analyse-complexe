...
\item La série diverge sur tout le cercle d'incertitude\footnote{Souvent malheureusement appelé \emph{cercle de convergence}.}.
...
\begin{enumerate}
\item  $a_n=2^n-3^n$; 
\item  $a_n=\left(1+(-1)^n\right)3^n$;
\item $a_n=n\sin(n)$;
\item $a_n$ est la $n$-ème décimale de $e$;\columnbreak
\item  $a_n$ est la suite de Fibonacci\footnote{Voir \url{https://oeis.org/A000045}}\\($0$, $1$, $1$, $2$, $3$, $5$, $8$, $13$  etc)
\item $a_n=\frac{n!}{n^n}$
\end{enumerate}
\end{multicols}
\end{exo}
