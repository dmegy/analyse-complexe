...
Si vous trouvez ce qui vous semble être une affirmation \emph{correcte} dans ce document, vérifiez avec vos camarades puis contactez-moi pour que je puisse l'enlever  de toute urgence.
\end{mdframed}

\section{Rappels (analyse, topo, nombres complexes, calculs diff etc)}



\begin{ideeFausse}
Soit $f$ une fonction continue et intégrable sur $\R_+$.
Alors $f(x) \xrightarrow[x\to +\infty]{} 0$.
\end{ideeFausse}

\begin{ideeFausse}
Soit $f : X\to Y$ continue. L'image d'un fermé est fermée. L'image d'un ouvert est ouverte. L'image réciproque d'un compact est compacte.
\end{ideeFausse}
...
\begin{ideeFausse}
Le module de $e^{iRe^{i\theta}}$ est $1$.
\end{ideeFausse}

\begin{ideeFausse}
$e^{-Re^{i\theta}}\to 0$ lorsque $R\to +\infty$.
\end{ideeFausse}

\begin{ideeFausse}
$\abs{\frac{1}{z^2+1}} \leq \frac{1}{1+|z|^2}$.
\end{ideeFausse}
...
Soit $\gamma \in \mathcal C^1([0,1],\C)$ un chemin.
Alors il existe $\tau\in[0,1]$ tq $\gamma(1) - \gamma(0) = \gamma'(\tau)$. 
\end{ideeFausse}


\begin{ideeFausse}
Soient $f$ et $g$ des fonctions continues par morceaux sur $\R_+$.
Si $|f|\leq |g|$ au voisinage de $+\infty$ et $\int_0^{+\infty} g$ est convergente, alors $\int_0^{+\infty} f$ est convergente.
\end{ideeFausse}

\begin{ideeFausse}
Soient $f$ et $g$ des fonctions continues par morceaux sur $\R_+$.
Si $f=o(g)$ au voisinage de $+\infty$ et $\int_0^{+\infty} g$ est convergente, alors $\int_0^{+\infty} f$ est convergente.
\end{ideeFausse}

\begin{ideeFausse}
Soient $f$ et $g$ des fonctions continues par morceaux sur $\R_+$.
Si $f\sim g$ au voisinage de $+\infty$ et $\int_0^{+\infty} g$ est convergente, alors $\int_0^{+\infty} f$ est convergente.
\end{ideeFausse}
...
\begin{ideeFausse}
Une fonction $f : U\to \C$ harmonique (i.e. de laplacien nul) est holomorphe.
\end{ideeFausse}
...
\begin{ideeFausse} \important
Soit $U\subset \C$ un ouvert. L'anneau $\mathcal O(U)$ est intègre.
\end{ideeFausse}

%\begin{ideeFausse} \important
%Soit $U\subset \C$ un ouvert connexe. L'anneau $\mathcal O (U)$ est factoriel.
%\end{ideeFausse}
...
%(Attention, la notion de compact à bord $\mathcal C^1_{pm}$ donnée en cours est assez restrictive.) % enlevé, inutile de faire stresser avec ça
...
\subsection{Séries de Laurent et singularités isolées}
...
\begin{ideeFausse}
$z\mapsto \frac{1}{e^{1/z}-1}$ a une singularité essentielle à l'origine.
\end{ideeFausse}
...
\begin{ideeFausse}
Une singularité isolée $z_0$ de $f$ est un pôle ssi $\exists k\in \N$ tel que $z_0$ est effaçable pour $z^kf(z)$.
\end{ideeFausse}
%

\begin{ideeFausse} 
Si $z_0$ est une singularité essentielle de $f$, alors pour tout $k\in\N$, $\frac{1}{(z-z_0)^k} = o(f)$ au voisinage de $z_0$. (\og Tout pôle est négligeable devant une singularité essentielle.\fg)
\end{ideeFausse}

\begin{ideeFausse}
Une fonction holomorphe sur $\C^*$ est méromorphe sur $\C$.
\end{ideeFausse}

\begin{ideeFausse}
Une fonction méromorphe a un nombre fini de pôles.
\end{ideeFausse}

\begin{ideeFausse}
La composée de deux fonctions méromorphes est méromorphe.
\end{ideeFausse}

\begin{ideeFausse}
Les fonctions méromorphes sur un ouvert $U$ forment un corps pour les lois usuelles.
\end{ideeFausse}
...
\begin{ideeFausse}
Si le résidu en une singularité isolée est nul, alors la singularité est effaçable.
\end{ideeFausse}

\begin{ideeFausse}
Si le résidu en une singularité isolée est nul, alors la singularité n'est pas essentielle.
\end{ideeFausse}
%
%\begin{ideeFausse}
%\end{ideeFausse}
